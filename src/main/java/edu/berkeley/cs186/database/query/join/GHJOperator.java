...
public class GHJOperator extends JoinOperator {
    ...
    /**
     * For every record in the given iterator, hashes the value at the column we're joining on and
     * adds it to the correct partition in partitions.
     *
    ...
    /**
     * Runs the buildAndProbe stage on a given partition. Should add any matching records found
     * during the probing stage to this.joinedRecords.
     */
    ...
    /**
     * Runs the grace hash join algorithm. Each pass starts by partitioning leftRecords and
     * rightRecords. If we can run build and probe on a partition we should immediately do so,
     * otherwise we should apply the grace hash join algorithm recursively to break up the
     * partitions further.
     */
    ...
    /**
     * Create an appropriate number of partitions relative to the number of available buffers we
     * have.
     *
    ...
    /**
     * Creates either a regular partition or a smart partition depending on the value of
     * this.useSmartPartition.
     *
     * @param left true if this partition will store records from the left relation, false
     *             otherwise
    ...
    /**
     * Creates a record using val as the value for a single column of type int. An extra column
     * containing a 500 byte string is appended so that each page will hold exactly 8 records.
     *
     ...
     * <p>
     * Come up with two lists of records for leftRecords and rightRecords such that SHJ will error
     * when given those relations, but GHJ will successfully run. createRecord(int val) takes in an
     * integer value and returns a record with that value in the column being joined on.
     * <p>
     ...
     * <p>
     * Come up with two lists of records for leftRecords and rightRecords such that GHJ will error
     * (in our case hit the maximum number of passes). createRecord(int val) takes in an integer
     * value and returns a record with that value in the column being joined on.
     * <p>
     * Hints: Both joins will have access to B=6 buffers and each page can fit exactly 8 records.
     *
     * @return Pair of leftRecords and rightRecords
